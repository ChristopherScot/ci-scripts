...
func printNext(o initOpts, c *config.Config, dir string, isCLI bool) {
	...
	if !isCLI && c.Spec {
		...
		// The FIRST publish of the TypeScript client is manual, and CI
		// cannot do it for you.
		//
		// CI publishes by npm trusted publishing (OIDC), which is
		// configured per PACKAGE on npmjs.com - and a package that has
		// never been published has no settings page to configure. So the
		// first version has to go up by hand; every later one rides the
		// spec's version bump automatically.
		//
		// Worth saying at init rather than leaving to be discovered:
		// until this runs, any service consuming the client resolves it
		// through an npm workspace, which links by SYMLINK. That works
		// in CI and dies in the container, where the link dangles and
		// node exits with ERR_MODULE_NOT_FOUND - a failure that points
		// nowhere near its cause.
		fmt.Println()
		fmt.Println("  - once the first commit is pushed, publish the TypeScript")
		fmt.Println("    client once, by hand:")
		fmt.Printf("      cd %s/clients/ts && npm publish\n", dir)
		fmt.Println("    then on npmjs.com, Settings > Trusted publishing, name this")
		fmt.Println("    repo and workflow. CI publishes every version after that.")
		fmt.Println()
	} else if !isCLI {
